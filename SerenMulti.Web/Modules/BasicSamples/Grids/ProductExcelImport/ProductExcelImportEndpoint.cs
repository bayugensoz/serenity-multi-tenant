...
using SerenMulti.Northwind.Repositories;
using System;
...
namespace SerenMulti.BasicSamples.Endpoints
{
    [Route("Services/BasicSamples/ProductExcelImport/[action]")]
    [ConnectionKey(typeof(MyRow)), ServiceAuthorize]
    public class ProductExcelImportController : ServiceEndpointMultiTenant
    {
        [HttpPost]
            ...
            [FromServices] IUploadStorage uploadStorage)
        {
            ...
            for (var row = 2; row <= worksheet.Dimension.End.Row; row++)
            {
                try
                {
                    ...
                    {
                        new ProductRepository(GetContext()).Create(uow, new SaveRequest<MyRow>
                        {
                            Entity = product
                        });
                    ...
                    {
                        new ProductRepository(GetContext()).Update(uow, new SaveRequest<MyRow>
                        {
                            Entity = product,
        ...
        // tambahan
        public ProductExcelImportController([FromServices] IUserRetrieveService userRetriever) : base(userRetriever) { }
